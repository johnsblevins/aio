...
New-AzDeployment -Name "Deploy-AIO-Load-Balancers-$random" -Location $location -TemplateFile '../bicep/main.bicep' -TemplateParameterFile '../parameters/main.parameters.json' -deploymentid $random -adminUsername $cred.UserName -adminPassword $cred.Password